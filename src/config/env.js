...
const envSchema = z.object({
   NODE_ENV: z.enum(['development', 'production', 'local', 'test']).default('local'),
   PORT: z.coerce
      ...
      .string({
         error: issue =>
            issue.input === undefined
               ? 'El host de la base de datos es obligatorio'
               : 'El host de la base de datos debe ser una cadena',
      })
      ...
      .string({
         error: issue =>
            issue.input === undefined
               ...
               : 'El nombre de la base de datos debe ser una cadena',
      })
   ...
   DB_PORT: z.coerce
      .number({
         error: issue =>
            issue.input === undefined
               ? 'El puerto de la base de datos es obligatorio'
               : 'El puerto de la base de datos debe ser un número',
      })
      ...
      .string({
         error: issue =>
            issue.input === undefined
               ...
               : 'El usuario de la base de datos debe ser una cadena',
      })
   ...
   DB_PASSWORD: z
      .string({
         error: issue =>
            issue.input === undefined
               ? 'La contraseña de la base de datos es obligatoria'
   ...
   AUTH_ACCESS_TOKEN_EXPIRATION: z
      ...
      .regex(/^\d+[smhd]$/, {
         message:
            'Debe ser un número seguido de [s:segundos, m:minutos, h:horas, d:días] (ej: 10m, 3s, 1h, 2d)',
      })
      .default('10m'),
   AUTH_REFRESH_TOKEN_EXPIRATION: z
      ...
      .default('3d'),
   SECRET_JWT: z.string().default('TRACKERGUY#12345'),
});
...
if (!parsedEnv.success) {
   const errors = parsedEnv.error.issues
      .map(err => `[ENV-ERROR] - ${err.path[0]}: ${err.message}`)
      .join('\n');
